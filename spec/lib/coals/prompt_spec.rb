...
RSpec.describe Coals::Prompt do
  ...
  before { allow($stdout).to receive(:puts).and_return nil; allow($stdout).to receive(:print).and_return nil }
  ...
  describe '#capture_selection' do
    it 'creates a menu with the options and returns the user selection' do
      ...
      result = dummy_instance.capture_selection(options: ['Brown', 'Grey', 'White', 'Tan'])
